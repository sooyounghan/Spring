...
2. 프로젝션 대상 : Entity, Embedded Type, Scalar Type(숫자, 문자 등 기본 데이터 타입)
   - SELECT m FROM Member m : Entity Projection
     + 💡 Entity Project으로 조회된 값들은 영속성 컨텍스트에서 관리
```java
package hellojpa;
...
```
   ...
   - SELECT m.address FROM Member m : Embedded Type Projection (💡 임베디드 값 타입은 소속된 타입 명시해야 되는 단점 존재)
```java
package hellojpa;
