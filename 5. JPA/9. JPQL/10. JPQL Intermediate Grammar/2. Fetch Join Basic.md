...
for(Member member : members) {
    ...
    System.out.println("username = " + member.getUsername() + ", " + "teamnmae = " + member.getTeam.name());
}
```
...
Hibernate: // 지연 로딩으로 인해 Member 엔티티만 조회
    ...
    FROM
        ...
        from
            Member m1_0
Hibernate: // Team 프록시 초기화 시점에 SQL 실행하여 1차 캐시 저장
    select
        t1_0.id,
        t1_0.name 
    from
...
username = 회원1, teamName = TeamA // 프록시 초기화로 SQL 실행
...
-----
1. SQL의 DISTINCT는 중보고딘 결과를 제공하는 명령
...
```
```
Hibernate: // Team 객체의 필드 값만 조회 (Members와 JOIN 되었을지라도)
    /* SELECT
        t 
    FROM
        Team t 
