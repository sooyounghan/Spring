...
```
   ...
   - 하지만, 문제 발생 (2가지)
     + persist와 flsuh를 하지 않으면, 현재 1차 캐시에 존재하므로 연관 관계가 아닌 값을 설정해줬을 때는 문제가 없음
...
Hibernate: 
    ...
    values
        (?, ?, ?)
```
   - 💡 하지만, 연관 관계의 주인이 아닌 값에 설정을 하지 않으면, team과 member는 1차 캐시에 존재함. 하지만, team의 컬렉션에는 member의 값을 저장하지 않았으므로, 빈 컬렉션 출력
```java
package hellojpa;
...
```
   ...
   - 💡💡💡 양방향 매핑 시 무한 루프 조심
     ...
     + 💡💡💡 예시) toString() : team ↔ members의 toString을 무한 호출
```java
package hellojpa;
