...
```
  ...
  - 서로 다른 고객이 같은 파일이름을 업로드하는 경우 기존 파일 이름과 충돌이 일어날 수 있으므로, 서버에서는 저장할 파일며과 겹치지 않도록 내부에서 관리하는 별도의 파일명이 필요
...
6. 멀티파트 파일을 서버에 저장하는 역할 담당
   - createStoreFile() : 서버 내부에서 관리하는 파일명은 유일한 이름을 생성하는 UUID를 사용해서 충돌하지 않도로 해야함
...
```
  ...
  - @GetMapping("/attach/{itemId}") : 파일을 다운로드할 때 실행
    ...
    + 💡 파일 다운로드 시에는 고객이 업로드한 파일 이름으로 다운로드하 하는 것이 좋음
